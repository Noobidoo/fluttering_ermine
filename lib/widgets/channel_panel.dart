...
class _VoiceParticipantRow extends StatefulWidget {
  ...
  @override
  State<_VoiceParticipantRow> createState() => _VoiceParticipantRowState();
}

class _VoiceParticipantRowState extends State<_VoiceParticipantRow> {
  void _showVolumeMenu(BuildContext context) {
    final voice = context.read<VoiceState>();
    final currentVolume = voice.getParticipantVolume(widget.identity, source: TrackSource.microphone);
                      ...
                      child: SliderTheme(
                        ...
                        child: Slider(
                          ...
                          onChanged: (v) {
                            ...
                            voice.setParticipantVolume(widget.identity, v, source: TrackSource.microphone);
                          },
                  ...
                  children: [
                    TextButton.icon(
                      onPressed: () {
                        ...
                        voice.setParticipantVolume(widget.identity, 1.0, source: TrackSource.microphone);
                      },
                      ...
                      TextButton.icon(
                        onPressed: () {
                          voice.setParticipantVolume(
                            widget.identity,
                            widget.isMuted! ? tempVolume.clamp(0.01, 2.0) : 0.0,
                        ...
                        label: Text(
                          widget.isMuted == true ? 'Unmute' : 'Mute',
                        ),
                        style: TextButton.styleFrom(
                          foregroundColor: Colors.white60,
                        ),
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(
                  color: widget.isSpeaking ? speakingColor : Colors.transparent,
                  width: 2,
                ),
              ...
              child: CircleAvatar(
                ...
                onBackgroundImageError:
                    widget.avatarUrl != null ? (_, _) {} : null,
            ...
            Icon(
              widget.isMuted == true ? Icons.mic_off : Icons.mic,
              size: 11,
              color:
                  widget.isMuted == true ? Colors.redAccent : speakingColor,
            ),
            const SizedBox(width: 4),
            Expanded(
              child: Text(
                widget.displayName + (widget.isLocal ? ' (you)' : ''),
                style: TextStyle(
                  fontSize: 12,
                  color: widget.isSpeaking ? Colors.white70 : Colors.white54,
                  fontWeight:
                      widget.isSpeaking ? FontWeight.w600 : FontWeight.normal,
                ),
                overflow: TextOverflow.ellipsis,
              ),
            ),
            if (!widget.isLocal && widget.isScreenSharing)
              GestureDetector(
                onTap: () => voice.toggleScreenShareSubscription(widget.identity),
                child: Container(
                  padding:
                      const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
                    color: voice.isScreenShareSubscribed(widget.identity)
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        voice.isScreenShareSubscribed(widget.identity)

...
void main() {
  ...
  testWidgets('voice join with deepfilter does not crash',
      (tester) async {
    final serverId = _env['VOICE_SERVER_ID'] ?? '';
    final channelId = _env['VOICE_CHANNEL_ID'] ?? '';
    if (serverId.isEmpty || channelId.isEmpty) {
      debugPrint('[integration] Skipping: set VOICE_SERVER_ID and VOICE_CHANNEL_ID in .env');
      return;
    }
    ...
    final server = serverState.servers.firstWhere(
      (s) => s.id == serverId,
      orElse: () {
        debugPrint('[integration] Server $serverId not in serverState.servers');
    ...
    final frameSize = await _channel.invokeMethod<int>('getFrameSize');
    expect(frameSize, greaterThan(0),
        reason: 'frame size must be valid');

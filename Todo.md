...
- [x] **Camera / video toggle** — `LocalParticipant.setCameraEnabled(bool)` with camera permission request; local PiP preview tile in `_VoiceChannelView`; camera toggle button in voice controls; preference persisted to `voice_camera_enabled`
